...
const SlideRam: React.FC = () => {
	...
	const basePresets = React.useMemo(
		() => (isProxyProvider(form.provider) ? PROXY_RAM_PRESETS : GAMEPLAY_RAM_PRESETS),
		[form.provider],
	);
	...
	React.useEffect(() => {
		...
		if (visiblePresets.some((preset) => preset.ram === form.ram)) return;
		updateField('ram', visiblePresets[0].ram);
	}, [form.ram, isAdvancedMode, updateField, visiblePresets]);
			...
			{isAdvancedMode ? (
				...
				<div className='flex flex-col gap-4'>
					{visiblePresets.map((preset) => {
						const selected = form.ram === preset.ram;
						return (
							<button
								key={preset.key}
								type='button'
								...
								<Card className={selected ? 'border-accent bg-accent/50' : ''}>
									<CardHeader>
